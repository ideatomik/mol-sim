...
# complexity_manager.gd's is_enabled("ligase_cofactor"), folded in with the
...
func _build() -> void:
	...
	ppi_link.width = tm.cofactor_fused_link_width
	ppi_link.default_color = tm.cofactor_fused_link_color
	...
	_add_bead(_ppi_group, Vector2.ZERO, bead_r, tm.cofactor_bead_color, "P")
	_add_bead(_ppi_group, Vector2(spacing, 0.0), bead_r, tm.cofactor_bead_color, "P")
...
func cleave() -> void:
	...
	if byproducts_visible:
		_ppi_tween.tween_property(_ppi_group, "position",
			_ppi_group.position + _tm.cofactor_discard_drift, _tm.cofactor_fade_duration)
		_ppi_tween.tween_property(_ppi_group, "modulate:a", 0.0, _tm.cofactor_fade_duration)
