...
#
# Organization: cross-cutting concerns (Fonts, Font Sizes) get their own
# groups instead of living wherever they were first needed; everything
# else stays grouped by FEATURE (Helicase Ring, Ligase, ...), with a
# Colors/Timing @export_subgroup pulled out inside a feature's group where
# that group mixes enough colors or durations with its shape/geometry
# fields to be worth separating. All 160 exports are referenced elsewhere
# purely by property name (never by declaration order or group/subgroup
# membership), so this organization is Inspector-display only.
# ==========================================
...
@export_group("Fonts")
## Font RESOURCES used across the simulation's on-screen text. Point sizes
## for these same fonts live in the separate "Font Sizes" group below —
## kept apart so "which font file" and "how big" can each be scanned as
## their own short list instead of interleaved through every feature that
## happens to draw text.
## Font for DNA nitrogen-base labels (A/T/C/G lettering).
...
## two skeletal atom-label sizes (which have no dedicated Font resource of
## their own — they render with whatever font molecule_structure_renderer.gd
## falls back to).
## DNA nitrogen-base labels — see base_label_font in Fonts.
@export var base_label_font_size: int = 14
## Cofactor bead symbols — see cofactor_label_font in Fonts.
@export var cofactor_label_font_size: int = 10
## Strand end markers — see marker_font in Fonts.
@export var marker_font_size: int = 14
## Shared enzyme name-label size (Enzyme Labels group).
...
## molecular_label_zoom_*_threshold in Molecular Structure → Zoom
## Thresholds. The wider/element-only band uses
## molecular_atom_label_font_size_element_only instead. Name kept as-is
## (not renamed to *_full_geometry) to avoid orphaning the existing
...
## Renamed from polymerase_rest_nudge_slots/polymerase_rest_move_duration —
## every other field in this group is clamp_-prefixed, only these two broke
## that pattern.
@export var clamp_rest_nudge_slots: float = 2.0
@export var clamp_rest_move_duration: float = 0.5
...
##
## See also: this is the hard ceiling the Molecular Structure → "Zoom
## Thresholds" chain (molecular_label_zoom_enter_threshold in particular)
## must stay strictly below, or that band becomes unreachable — a past bug
## happened exactly this way (placeholder 9.5 exceeded this ceiling of 8.0).
...
## — NOT the phosphodiester backbone, see molecular_backbone_bond_
...
## Unlisted elements (anything not N/C/O/P above) fall back to
## molecular_bond_color in "Bond Lines" above.
...
## in case a future pass wants it back for some other purpose. Still read
## by scripts/molecule_geometry_diagnostics.gd's debug dump, so genuinely
## not dead from that caller's point of view even though the main render
