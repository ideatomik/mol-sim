...
func _ready():
	...
	initialize_simulation(dna_sequence.to_string())
...
func initialize_simulation(sequence: String):
	...
	queue_redraw()
	print("[INIT] Simulation initialized with %d bases: %s" % [num_nucleotide_slots, dna_sequence.to_string()])
...
func toggle_play():
	...
	if not manual_override:
		# If we're at DONE, reset to start when pressing play
		if phase == Phase.DONE:
			scrub_to_nucleotide_index(0)
		# Make sure the synthesis circle is visible when playing
...
func scrub_to(progress: float):
	...
	# Clear and rebuild synthesized bases
	...
	# they'll respawn progressively as synthesis completes again.
	...
	for i in range(synth_count, num_nucleotide_slots):
...
func _spawn_hydrogen_bonds(template_index: int) -> Node2D:
	...
	# Inset the bond endpoints by the base body radius so lines don't
	# visually pass through the nucleotide circles.
