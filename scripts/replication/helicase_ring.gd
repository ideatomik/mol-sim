...
## already uses. Godot's own double_click detection on the press event, so
## no manual timing here; a double-click short-circuits BEFORE the drag path
...
## and release — including the incidental few px of jitter between a
## double-click's two presses — committed to a drag and fired
## scrub_drag_delta, which pauses playback unconditionally regardless of
## whether the resulting index actually changed. Found via [FOLLOWCLICK]
## prints: LP's smaller click region made it near-guaranteed, helicase's
## larger one made it intermittent — same bug, different odds.
