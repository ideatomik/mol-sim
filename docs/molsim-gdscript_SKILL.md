...
### "Temporary" is not a priority level

A render/simulation state does not inherit lower correctness priority from
being time-bounded, pre-fork, or resolved once some later event occurs.
Whether a state deserves full correctness effort is a pedagogical/user-
facing question — how much a student actually sees and relies on it — never
a lifecycle question. The self-paired template state (both template strands
still paired to each other, before either has a real synthesized partner —
see `MolecularStructureDesign.md`'s dedicated entry and
`MolecularStructure_BasePairExpansion.md`'s Bug V/W thread) is the concrete
case this rule was written for: it was repeatedly discussed as lower
priority because it "only" lasts until the fork arrives, while in fact it is
the state a student sees during any enzyme-free pan of the assembled DNA —
arguably the single cleanest teaching view in the whole simulator, seen for
as long as the student chooses to sit in it. Don't infer render/correctness
priority from how long a state lasts in simulated time; ask how much of it
a real viewer actually spends looking at.
